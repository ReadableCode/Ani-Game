...
def create_data_set_in_domo(name, description, df):
    """
    This function will create a data set in domo using the APIa d initialize it
    ...
    print_logger(f"Finished Creating Data Set in Domo: id is {created_table_id}")
...
def update_data_set_in_domo(
    ...
    print_logger(f"Finished writing to Domo: {id} on try {try_num} of {retries}")
    return df_return
